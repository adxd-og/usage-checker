...
/// Walking a process up to launchd to find the terminal or IDE it runs under.
///
/// Compiled into the app *and* the `OmeletteHook` target (one `- path:` entry per
/// target in project.yml): the hook walks its own parents, and the app walks the
/// parents of a tmux client, and both have to reach the same verdict from the same
/// table of bundle ids. Foundation only — the hook links nothing else.
enum HostWalk {
    /// What the walk found: the app a click should activate, and the first
    /// controlling terminal seen on the way up.
    ...
    /// Terminals and IDEs the app knows how to bring to the front.
    ...
    /// activate something. Under tmux nothing on the chain has a bundle id at all and
    /// the answer is empty but for the tty — which is the pane's, not a terminal's.
    ///
    /// `read` is injectable for the tests; production reads the kernel.
    static func describe(from pid: pid_t, read: (pid_t) -> ProcessRecord? = { ProcessRecord.read($0) }) -> Terminal {
        var result = Terminal()
        var known: (pid: pid_t, bundleID: String)?
        var outermostApp: (pid: pid_t, bundleID: String)?
        var current = pid
        var hops = 0
        while current > 1, hops < maxHops, let record = read(current) {
            hops += 1
