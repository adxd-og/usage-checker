...
### Fixed
- **Dashboard text no longer comes back upside down after sleep.** On macOS 27.0
  the selectable text in the detail column was flipped when the Mac woke until a
  provider switch redrew it; the dashboard now redraws the column itself once
...
- **Clicking an agent that runs inside tmux jumps to its pane.** The terminal
  ...
  the search for the terminal ends at the tmux server. Terminal and iTerm2 also
  select the right tab. A tmux nobody is attached to still opens the folder.
  Closes #8.
...
### Changed
- **Install this one by hand.** The project moved to `github.com/adxd-og/usage-checker`
  and its update feed to `adxd-og.github.io`; GitHub does not redirect the old feed
  address, so builds up to 2.2.1 can no longer see new versions. Download the DMG
  once — from 2.2.2 on, automatic updates work again.
